"""Build the ``spawn launch`` / ``spawn terminate`` argv for a Snakemake job.

Ported verbatim from cwl-spawn/miniwdl-spawn/nf-spawn: the spawn CLI contract is
engine-agnostic. ``--on-complete terminate`` (instance self-destructs when the
job signals done), ``--user-data-file`` (NOT ``--user-data``), and the
